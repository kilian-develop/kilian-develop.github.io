...
tags:
  ...
  - multimodule
excerpt: 도메인 중심 설계를 통해 RDB, MongoDB 등 인프라 변경에 영향받지 않는 비즈니스 로직을 만드는 방법. DiscountPolicy 전략패턴 예제로 DB 스키마 진화를 보여주고, 멀티모듈 구조와 runtimeOnly 의존성으로 컴파일 타임 격리를 구현하는 실전 가이드입니다.
---
# 하위 레이어에 의존적인 아키텍처
...
할인이라는 역할을 인터페이스로 추상화하면 문제를 깔끔하게 해결할 수 있습니다:

```java
public interface DiscountPolicy {
    boolean isValid();
    BigDecimal calculateDiscountAmount(BigDecimal originalAmount);
}
```

각 할인 정책이 이 인터페이스를 구현합니다:
...
public class CouponDiscount implements DiscountPolicy {
    ...
    public CouponDiscount(String couponCode, BigDecimal discountAmount, boolean isExpired) {
...
Order 도메인은 **역할과 책임 중심**으로 설계됩니다. 필요한 필드만 가지고 있고, 할인 정책의 세부사항을 알 필요가 없습니다:
...
public class Order {
    ...
    public Order(Long id, String orderNumber, BigDecimal originalAmount) {
        this.id = id;
        this.orderNumber = orderNumber;
    ...
    // 할인액을 받아서 최종 금액을 계산
    // 어떤 할인 정책인지는 알 필요 없음
    public BigDecimal calculateFinalAmount(BigDecimal discount) {
...
DiscountPolicy는 OrderId를 간접 참조하여 해당 주문에 적용되는 할인 정책들을 나타냅니다:

```java
public interface DiscountPolicy {
    boolean isValid();
    Long getOrderId();  // OrderId에 대한 간접 참조
...
이제 각 구현체는 추상클래스를 상속하기만 하면, OrderId 관리와 getOrderId() 메서드를 자동으로 얻게 됩니다. 각 구현체는 자신의 할인 로직(isValid, calculateDiscount)만 구현하면 됩니다.
...
### 핵심: 영속성 레이어에서의 추상화 처리

여기서 중요한 점은 **DiscountPolicyRepository에서 DB 구조를 어떻게 설계하든, 도메인 계층에서는 DiscountPolicy 인터페이스를 통해서만 다룬다**는 것입니다.

**Step 1: 초기 설계 - 모든 할인 정보를 한 테이블에 저장**
...
**Step 2: DB 스키마 변경 - 할인 정책을 타입별 테이블로 분리**
...
// 타입 정보를 저장하는 별도의 테이블
@Entity
@Table(name = "discount_policy_types")
public class DiscountPolicyTypeEntity {
    @Id
    private Long id;
    private Long orderId;
    private String policyType;  // FIXED, PERCENTAGE, COUPON
...
**결정적인 점: DiscountPolicyRepository의 findByOrderId() 메서드만 변경, 도메인과 서비스는 그대로**
...
@Repository
public class DiscountPolicyRepositoryImpl implements DiscountPolicyRepository {
    private final DiscountPolicyTypeJpaRepository typeRepository;
    ...
    @Override
    public List<DiscountPolicy> findByOrderId(Long orderId) {
        List<DiscountPolicyTypeEntity> typeEntities = typeRepository.findByOrderId(orderId);

        return typeEntities.stream()
            .map(typeEntity -> buildDiscountPolicy(typeEntity, orderId))
            .collect(Collectors.toList());
    }

    private DiscountPolicy buildDiscountPolicy(DiscountPolicyTypeEntity typeEntity, Long orderId) {
        // 타입에 따라 적절한 테이블에서 조회
        switch(typeEntity.getPolicyType()) {
            case "FIXED":
                FixedDiscountPolicyEntity fixed = fixedRepository.findByOrderId(orderId)
                    .stream()
                    .findFirst()
                    .orElse(null);
                if (fixed != null) {
                    return new FixedAmountDiscount(fixed.getOrderId(), fixed.getAmount());
                }
                break;

            case "PERCENTAGE":
                PercentageDiscountPolicyEntity percentage = percentageRepository.findByOrderId(orderId)
                    .stream()
                    .findFirst()
                    .orElse(null);
                if (percentage != null) {
                    return new PercentageDiscount(percentage.getOrderId(), percentage.getPercentage());
                }
                break;

            case "COUPON":
                CouponDiscountPolicyEntity coupon = couponRepository.findByOrderId(orderId)
                    .stream()
                    .findFirst()
                    .orElse(null);
                if (coupon != null) {
                    return new CouponDiscount(coupon.getOrderId(),
...
**도메인 중심 설계의 장점:**
- 영속성 레이어에서만 DB 구조를 알고 있음
- 서비스와 도메인은 인터페이스를 통해 추상화됨
- 새로운 할인 정책 추가 시 Order, OrderService는 전혀 변경되지 않음
- 오직 영속성 레이어의 deserialize 메서드에만 switch 문 추가
...
**order-service에서 직접 OrderRepositoryImpl를 호출할 수 없습니다:**
...
왜냐하면 order-persistence는 runtimeOnly 의존성이므로, order-service가 컴파일될 때 OrderRepositoryImpl 클래스를 찾을 수 없기 때문입니다.
...
이제 MongoDB로 마이그레이션해야 한다면?

**order-mongodb** 모듈을 새로 만들고:
...
public class MongoOrderRepository implements OrderRepository {
    ...
    public Order save(Order order) {
        OrderDocument doc = new OrderDocument();
        doc.setOrderNumber(order.getOrderNumber());
        doc.setOriginalAmount(order.getOriginalAmount());
        mongoDao.save(doc);
        return order;
    }
    ...
    public Order findById(Long id) {
        return mongoDao.findById(id)
            .map(doc -> new Order(doc.getId(), doc.getOrderNumber(),
                                 doc.getOriginalAmount(), new FixedAmountDiscount(BigDecimal.ZERO)))
            .orElseThrow();
    }
}
```

**order-service의 build.gradle을 변경하기만 하면 됩니다:**

```gradle
dependencies {
    implementation project(':order-domain')
    runtimeOnly project(':order-mongodb')  // JPA → MongoDB로 교체
}
```

**order-service와 order-application의 코드는 전혀 변경되지 않습니다!** 인터페이스에만 의존했기 때문입니다.